        ...
        for rubro_idx, rubro in enumerate(self.rubros, 1):
            ...
            if rubro['equipos']:
                for equipo in rubro['equipos']:
                    ...
                    if equipo.get('cantidad') is not None:
                        c = ws.cell(row=current_row, column=3, value=equipo.get('cantidad'))
                        c.number_format = fmt_number
                    if equipo.get('tarifa') is not None:
                        c = ws.cell(row=current_row, column=4, value=equipo.get('tarifa'))
                        c.number_format = fmt_number
                    if equipo.get('costo_hora') is not None:
                        c = ws.cell(row=current_row, column=5, value=equipo.get('costo_hora'))
                        c.number_format = fmt_number
                    if equipo.get('rendimiento') is not None:
                        c = ws.cell(row=current_row, column=6, value=equipo.get('rendimiento'))
                        c.number_format = fmt_rendimiento
                    c = ws.cell(row=current_row, column=7, value=equipo.get('costo', rubro['subtotal_m']))
                    c.number_format = fmt_number
                    c = ws.cell(row=current_row, column=8, value=equipo.get('peso_relativo'))
                    c.number_format = fmt_peso_relativo
                    c = ws.cell(row=current_row, column=9, value=equipo.get('cpc', '4299217233'))
                    c.number_format = fmt_text
                    ws.cell(row=current_row, column=10, value=equipo.get('np_ep_nd', 'ND'))
                    c = ws.cell(row=current_row, column=11, value=equipo.get('vae_pct', 0.4))
                    c.number_format = fmt_vae_pct
                    c = ws.cell(row=current_row, column=12, value=equipo.get('vae_elemento'))
            ...
            ws.cell(row=current_row, column=7, value=rubro['total_costo_directo']).border = thin_border
            ws.cell(row=current_row, column=8, value=1).border = thin_border  # 100%
            ...
            ws.cell(row=current_row, column=12, value=rubro['vae_total']).border = thin_border
...
def convert_pdf_to_excel(pdf_path, output_path=None):
    ...
    if output_path is None:
        ...
        output_path = os.path.join(os.path.dirname(pdf_path), f"{base_name}_CONVERTIDO.xlsx")
